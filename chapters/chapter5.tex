...
图\ref{fig:uto3_tropomi}为2019年6--8月TROPOMI云切片方法得到的不同高度O$_3$的平均浓度，
其中有效范围比图\ref{fig:utno2_tropomi}更广，尤其是在中高云条件下的中纬度地区（图\ref{fig:uto3_tropomi} b--d）。
...
对于热带地区，非洲中部（20$^{\circ}$ W--50$^{\circ}$ E，0--20$^{\circ}$ N）的O$_3$浓度比同纬度地区的O$_3$浓度高，尤其是在中云和高云条件下可高出40\%--48\%，该现象与\citet{Cooper.2013}研究中215 hPa处MLS的O$_3$观测结果相符。
他们的研究指出，低O$_3$事件经常发生在热带西太平洋，很少发生在热带其他地方，
而热带海洋区域在低O$_3$事件和高云的频率之间存在一致的空间特征，表明这些事件来自于对流。
虽然非洲中部也是强对流频发地区，但陆地边界层O$_3$浓度往往高于海洋，因此深对流产生较少的低O$_3$事件。

为了进一步利用MERRA2-GMI分析动力输送和化学反应在其中的作用，我们首先将各个高度的MERRA2-GMI模式结果与TROPOMI观测数据相比较。
图\ref{fig:uto3_merra2}为2019年6--8月与TROPOMI对应的MERRA2-GMI O$_3$模拟结果，
图\ref{fig:uto3_delta}为TROPOMI与MERRA2-GMI在各层的结果之差。
总体上，非洲中部的$\Delta$ O$_3$在中云和高云条件下可小于-20 pptv（图\ref{fig:uto3_delta} a--d），中纬度地区的$\Delta$ O$_3$在高云条件下可大于 40 pptv（图\ref{fig:uto3_tropomi} b），
该两处之外的$\Delta$ O$_3$绝对值小于10。
...
\begin{figure}[!htbp]
    ...
    The O$_3$ vertical mixing ratios derived from the cloud-slicing results of TROPOMI O$_3$ observations at the northern middle and low latitudes for June--August in 2019.
    }
...
\begin{figure}[!htbp]
    ...
    Same as Fig. \ref{fig:uto3_tropomi} but for MERRA2-GMI.
    }
...
\begin{figure}[!htbp]
    ...
    Differences between Fig. \ref{fig:uto3_tropomi} and Fig. \ref{fig:uto3_merra2}.
    }
...
我们将MLS于261 hPa高度观测的O$_3$浓度与MERRA2-GMI的模拟结果，在晴空和有云条件下进行对比。
如图\ref{fig:mls_o3_261hpa}所示，在低纬度地区两者均表明有云条件下海洋上O$_3$浓度较晴空降低5\%--60\%，而非洲中部和印度北部的O$_3$浓度较晴空增大1\%--46\%。
晴空条件下两者在中纬度地区的O$_3$浓度一致，但有云时两者的变化相反，具体而言，MLS观测显示有云条件下261 hPa高度的O$_3$浓度比晴空时的浓度高，而MERRA2-GMI模拟显示O$_3$浓度比晴空时的浓度低。
由于MERRA2-GMI模拟结果与MLS观测在30--60$^{\circ}$ N区域内差异需要将来进一步地结合模式和探空数据进行研究，下一节的动力和化学贡献分析限定于0--30$^{\circ}$ N区域内。
...
\begin{figure}[!htbp]
    ...
    The mean O$_3$ concentrations at 261 hPa with clear (a--b) and cloudy (c--d) conditions at the northern middle and low latitudes for June--August in 2019. The percentage differences are shown in panel (e) and (f).
...
而MERRA2-GMI的模拟结果在该高度不存在O$_3$峰值，即MERRA2-GMI中上对流层LNO$_2$的差异导致O$_3$的低估。
对于清洁的太平洋地区，两者趋势一致，且误差小于5 pptv。
...
\begin{figure}[!htbp]
    ...
    其中廓线的误差棒为平均值$\pm$标准差。\\
    Regional average NO$_2$ profiles obtained by MERRA2-GMI (blue) and TROPOMI cloud slice algorithm (orange)
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=10cm]{./figures/uto3_tendency.pdf}
    \caption{
    2019年6--8月北半球中低纬度MERRA2-GMI模拟的300 hPa高度O$_3$的平均趋势：
    （a）动力（b）物理（c）化学。\\
    The mean tendency of O$_3$ simulated by MERRA2-GMI at 300 hPa due to (a) dynamics, (b) physics, and (c) chemistry at the northern middle and low latitudes for June--August in 2019.
    }
