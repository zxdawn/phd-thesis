...
以TROPOMI的NO$_{\ch{2}}$产品为例，对于不同的$p_{cloud}$至少需要两个邻近的云上$V_{\ch{NO2}}$（$V_{\ch{NO2Vis}}$，图\ref{fig:cloud-slicing_schematic}a），这两个测量的结果显示在图\ref{fig:cloud-slicing_schematic}b 中的$p_{cloud}$-VCD 坐标平面中。
...
虽然云切片方法无需知道具体的平流层NO$_{\ch{2}}$柱浓度，
...
利用\ref{sec:cloud-slicing}节的云切片算法，可进一步得到每个高度层间的NO$_{\ch{2}}$浓度，即NO$_{\ch{2}}$廓线。
...
除了各层O$_{\ch{3}}$浓度的地理分布之外，本研究选取了区域内云切片有效层数为6层的格点（位于中国南部、印度中部、美国东南部以及太平洋，图\ref{fig:no2_ltngcount}a），
...
该结果解释了臭氧探空观测的对流层上层O$_3$浓度增大的现象，揭示了化学贡献在对流整个生命期中的主导地位。
...
本研究将开启和关闭LNO排放所得到的中国东南部O$_{\ch{3}}$累计物理变化速率（IPR）进行对比，
...
接着，本研究以受LNO$_{\ch{2}}$影响更大的2019年为例，利用积分反应速率（IRR）来评估LNO$_{\ch{x}}$对$\Delta$O$_{\ch{3}}$的影响，以及O$_{\ch{3}}$变化的化学机制。
...
\item TROPOMI的云切片结果表明，陆地上对流层顶--330 hPa高度间的NO$_{\ch{2}}$浓度为450--570 hPa高度间的$\sim$2倍，570 hPa高度以下的NO$_{\ch{2}}$浓度随高度而逐渐下降，
...
结果表明MERRA2-GMI和TM5的LNO$_{\ch{2}}$排放或参数化对流垂直传输污染物的强度可能偏低，
...
鉴于MERRA2-GMI与TROPOMI的相近，本研究将MLS在261 hPa高度层高估的O$_{\ch{3}}$浓度归因于平流层过度的外推。
...
此外本研究将中国东南部的对流分为对流核心区和层云区，结果表明对流核心区的动力输送作用为层云区的$\sim$2倍，层云区的O$_3$变化受核心区的输送贡献所控制。
