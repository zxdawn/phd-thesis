...
闪电氮氧化物的重要性、深对流和闪电活动关系密切、深对流影响污染物垂直分布

氮氧化物[NO$_{\ch{x}}$ = 一氧化氮（NO）+ 二氧化氮（NO$_{\ch{2}}$）]是地球大气中重要的痕量气体，
对于调节臭氧（O$_{\ch{3}}$）及其他痕量气体浓度至关重要。
平流层中，NO$_{\ch{x}}$的主要来源是N$_{\ch{2}}$O的光解，它在O$_{\ch{3}}$的消耗中起着催化作用。
对流层中，人为排放的NO$_{\ch{x}}$约为 48 Tg氮每年（Miyazaki 等人，2016 年），
自然来源主要包括土壤排放和闪电\citep{Finlayson-PittsBarbaraJ..2000,Vinken.2014}。
然而，自然源的排放量存在很大的不确定性，其中闪电NO$_{\ch{x}}$（LNO$_{\ch{x}}$）的全球产量约为2--8 Tg氮每年\citep{Schumann.2007}。

具体而言，闪电在高温下通过分解氧分子（O$_{\ch{2}}$）和氮分子（N$_2$）产生NO，
接着NO迅速氧化为NO$_{\ch{2}}$并达到平衡\citep{Zeldovich.1967}。
虽然LNO$_{\ch{x}}$对总NO$_{\ch{x}}$收支的贡献很小，但是鉴于其寿命相对较长，且对流层上层NO$_{\ch{x}}$背景浓度更低，
所以LNO$_{\ch{x}}$对对流层上层NO$_{\ch{x}}$浓度的贡献很大。
在亚热带和热带地区，LNO$_{\ch{x}}$约占自由对流层NO$_{\ch{x}}$的70\%，在高纬度地区的夏季，该比例也可达20\%以上 \citep{Galloway.2004,Allen.2010}。
因为NO$_{\ch{2}}$可进一步在对流下风向的晴空区发生强烈光解，所以500--300 hPa之间的O$_{\ch{3}}$浓度可增加10--20 ppbv \citep{DeCaria.2005,Hauglustaine.2001}。
如果全球化学模式中LNO$_{\ch{x}}$的产量从 5 Tg氮每年翻倍至 10 Tg氮每年，则计算得到的全球对流层O$_{\ch{3}}$增加7--12 \%（Brasseur 等人，1996 年；Labrador 等人，2005 年）。

Huntrieser 2015

由于闪电与对流活动关系密切，因此很难将O$_{\ch{3}}$的变化与雷暴中的LNO$_{\ch{x}}$排放和地表NO$_{\ch{x}}$排放区分开\citep{Schultz.2004}。
在深对流系统中，污染物从地面传输到对流层上层只需要几分钟至一个小时\citep{Chatfield.1984,Dickerson.1987,Pickering.1989,Skamarock.2000,Yin.2002}。
在低纬度地区，对流每天以约8\%的速率向对流层上层输送，
这与由光化学过程控制的氢氧化物[HO$_{\ch{x}}$ = 羟基（OH）+ 过氧化氢自由基（HO$_{\ch{2}}$）]和NO$_{\ch{x}}$的产率相当。
深对流甚至可以直接将边界层大气输送至对流层上层和平流层下层（UTLS）\citep{Prather.1997}，
其中O$_{\ch{3}}$前体物的垂直传输显著提高了对流层上层云砧出流区O$_{\ch{3}}$的产量\citep{Pickering.1990,Pickering.1992,Pickering.1992a}。
...
闪电外推法将全球LNO$_{\ch{x}}$的产率定义为：每次闪电LNO$_{\ch{x}}$的产率与全球闪电频率的乘积\citep{Lawrence.1995}。
