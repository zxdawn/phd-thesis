...
\subsection{闪电与深对流的关系} \label{sec:lightning_convection}
...
\citet{Price.1992}提出一种常用的基于云顶高度（CTH）计算雷电密度的方法，
该方法基于\citet{Vonnegut.1963}和\citet{Williams.1985}的理论研究，使用闪电观测和卫星数据得到下面的参数化：

\begin{equation} \label{eq:ltng_cth}
F_\mathrm{l} = 3.44\times10^{-5}H^{4.9};
F_\mathrm{o} = 6.2\times10^{-4}H^{1.73}
\end{equation}
其中F是总闪频率（flash min$^{-1}$），H是云顶高度（km），下标l和o分别用于陆地和海洋。
...
\end{equation}
指数k是根据卫星数据得出，其中适用于大陆深对流的k值为4.5\citep{Price.1992}。
尽管此两种参数化具有基于观测的优势，但其在季节内和季节间关系与全球变暖背景下的关系并不相同，所以需要开发具有物理基础的参数化，并与观测进行对比再应用于全球气候模式。
由于北美大陆是全球闪电的主要分布地区之一（图\ref{fig:lisotd_season}），
\citet{Romps.2014}利用该地区的观测数据和再分析资料，
提出单位面积的闪电密度与对流有效位能和降水率的乘积（CAPE$\times$P）成正比。
该方案结合了闪电速率与降水率的线性关系\citep{Battan.1965,Petersen.1998,Tapia.1998}和闪电与CAPE的正相关关系\citep{Williams.2005a}，具体表达式如下：
...
此外，比例常数$\eta$/E包含无量纲的转换效率$\eta$和单次闪电释放的能量E（J），
$\eta$是闪电单位面积的释放功率与单位时间单位面积的CAPE之比。
虽然非感应起电机制是电荷分离并产生闪电的主要途径\citep{Barthe.2007,Saunders.2008}，
但是早期的研究通常只有间接相关的对流特性被引入大尺度的闪电参数化当中。
如今模式中的云冰模拟效果得到改善，故可以利用冰晶的向上通量开发详细的闪电参数化。
...
\end{equation}
其中F$_{\ch{l}}$和F$_{\ch{o}}$分别是陆地和海洋的闪电密度（flash m$^{-2}$ s$^{-1}$）。
...
当没有对流云顶被诊断出来时，闪电密度也设为0。
在全球变暖的背景下，$CAPE$$\times$$P$预测北美大陆的闪电数将增加12$\pm$5\% K$^{-1}$ \citep{Romps.2014}，而IFluxP预测闪电将仅增加3.4\%K$^{-1}$。

2）260 K等温线上的对流冰通量（IFluxT）

\citet{Romps.2019}指出除夏季月份外，IFluxP参数化显著高估了美国西部山区的闪电。
IFluxP存在的另一个问题是，固定等压线上的定义不适合研究全球变暖的影响，
因为闪电是由云的混合相区域中的起电所引起\citep{Williams.1991}，
并且该区域以273 K和240 K等温线为界。
因此全球变暖条件下起电层将随等温线改变，而不是等压线。
\citet{Romps.2019}通过在云分辨模型中详细对比了CAPE$\times$P和IFluxT的差异，
结果表明由全球变暖引起的高CAPE值和强上升气流速度可能导致使用CAPE$\times$P模拟的热带闪电大量增加，
