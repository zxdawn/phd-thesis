...
针对中国东部的强对流天气（\ref{sec:china}节和\ref{sec:convec_impacts}节），于2019年7月25日和2020年9月1日
在南京国家基准气候站（31.93$^{\circ}$ N，118.90$^{\circ}$ E）共释放了五个臭氧探空仪。
为了研究对流对O$_{\ch{3}}$垂直分布的影响，本研究分别在对流发生前和对流发生中/对流发生后各释放一次探空。
...
本研究中LNO$_{\ch{x}}$产率计算所使用的闪电数据集有以下几种，并针对不同的地区进行了详细的探测效率修正。

在第\ref{chapter:retrieval}章美国大陆的研究中，本研究使用了整体闪电探测网（ENTLN）数据。
...
在\ref{sec:china}节中国东部的研究中，融合了三种闪电数据集：中国国家闪电探测网[CNLDN，\citet{Yang.2015}]、
...
本研究使用NASA标准产品v3.0\citep{Krotkov.2017}中的云产品（云高和云辐射分数）和NO$_{\ch{2}}$产品作为LNO$_{\ch{2}}$反演算法的输入。
...
本研究使用Sentinel-5P产品算法实验室（S5P-PAL）再处理产品（v2.3.1）中的云产品（云高和云辐射分数）和NO$_{\ch{2}}$产品。
...
本研究除了使用卫星遥感数据之外，也利用WRF-Chem大气化学模式详细分析了NO$_{\ch{x}}$和O$_{\ch{3}}$的垂直分布变化，并提供反演NO$_{\ch{2}}$柱浓度所用的先验廓线。
具体而言，WRF-Chem是将WRF模式与大气化学模块结合起来的大气化学模式，
...
由于本研究为了更好的模拟出深对流系统，不同的个例使用了不同的微物理、化学及闪电参数化方案，其中闪电参数化可用闪电同化代替，详见\ref{sec:amf_definition}节和\ref{sec:model_settings_china}节。

\subsection{MERRA2-GMI模拟数据集}

现代研究和应用回顾分析（MERRA-2）GMI 模式是基于Goddard地球观测系统（GEOS）框架\citep{Molod.2015}，
数据下载地址为\url{https://acd-ext.gsfc.nasa.gov/Projects/GEOSCCM/MERRA2GMI/}。
本研究不仅使用MERRA2-GMI模拟数据集中的O$_{\ch{3}}$浓度产品，也利用O$_{\ch{3}}$趋势分析量和云量信息来评估对流对O$_{\ch{3}}$的具体影响。
具体而言，模式使用的再分析资料为MERRA-2\citep{Gelaro.2017}，
化学机制为GMI（Global Modeling Initiative）平流层和对流层化学相结合的机制\citep{Duncan.2007,Oman.2013,Nielsen.2017}。
